...
#[tokio::test]
async fn latest_for_hyperlinks_ignores_removed_legacy_kinds() {
    let connection = new_connection().await;
    ...
    connection
        .execute_unprepared(
            r#"
                INSERT INTO hyperlink_processing_job (id, hyperlink_id, kind, state, error_message, queued_at, started_at, finished_at, created_at, updated_at)
                VALUES
                    (1, 5, 'snapshot', 'succeeded', NULL, '2026-03-14 00:00:01', NULL, '2026-03-14 00:00:02', '2026-03-14 00:00:01', '2026-03-14 00:00:02'),
                    (2, 5, 'tag_reclassify', 'failed', 'removed', '2026-03-14 00:00:03', NULL, '2026-03-14 00:00:04', '2026-03-14 00:00:03', '2026-03-14 00:00:04');
            "#,
        )
        .await
        .expect("processing job seed data should insert");

    let latest = latest_for_hyperlinks(&connection, &[5])
        .await
        .expect("latest jobs should load");
    let job = latest.get(&5).expect("supported latest job should exist");
    assert_eq!(job.kind, HyperlinkProcessingJobKind::Snapshot);
}

#[tokio::test]
async fn delete_removed_job_rows_deletes_legacy_kinds() {
    let connection = new_connection().await;
    ...
    connection
        .execute_unprepared(
            r#"
                INSERT INTO hyperlink_processing_job (id, hyperlink_id, kind, state, error_message, queued_at, started_at, finished_at, created_at, updated_at)
                VALUES
                    (1, 8, 'snapshot', 'queued', NULL, '2026-03-14 00:00:01', NULL, NULL, '2026-03-14 00:00:01', '2026-03-14 00:00:01'),
                    (2, 8, 'tag_classification', 'failed', 'removed', '2026-03-14 00:00:02', NULL, '2026-03-14 00:00:03', '2026-03-14 00:00:02', '2026-03-14 00:00:03'),
                    (3, 8, 'tag_reclassify', 'failed', 'removed', '2026-03-14 00:00:04', NULL, '2026-03-14 00:00:05', '2026-03-14 00:00:04', '2026-03-14 00:00:05');
            "#,
        )
        .await
        .expect("processing job seed data should insert");

    let affected = delete_removed_job_rows(&connection)
        .await
    ...
    let rows = connection
        ...
        .await
        .expect("remaining rows should load");
    assert_eq!(rows.len(), 1);
    assert_eq!(
        rows[0]
            .try_get_by_index::<String>(0)
            .expect("kind should decode"),
        "snapshot"
    );
}
