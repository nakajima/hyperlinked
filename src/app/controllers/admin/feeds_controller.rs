...
use crate::{
    app::{helpers::admin_dashboard::ADMIN_TAB_FEEDS_PATH, models::rss_feed as rss_feed_model},
    entity::rss_feed,
    server::{
        context::Context,
...
async fn index(State(state): State<Context>, headers: HeaderMap) -> axum::response::Response {
    let feeds = match rss_feed_model::list(&state.connection).await {
        Ok(feeds) => feeds,
        Err(err) => {
    ...
    for feed in feeds {
        let hyperlink_count = rss_feed_model::hyperlink_count_for_feed(&state.connection, feed.id)
            .await
            .unwrap_or(0);
...
async fn sync(
    ...
    match rss_feed_model::sync_by_id(&state.connection, id, state.processing_queue.as_ref()).await {
...
async fn toggle_active(
    ...
    match rss_feed_model::toggle_active(&state.connection, id).await {
        Ok(Some(feed)) => {
...
async fn delete(
    ...
    match rss_feed_model::delete_by_id(&state.connection, id).await {
